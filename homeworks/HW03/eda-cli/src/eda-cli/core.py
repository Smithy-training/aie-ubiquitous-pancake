...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame, df: pd.DataFrame) -> Dict[str, Any]:
    ...
    # Constant columns
    constant_columns = [col.name for col in summary.columns if col.unique == 1 and n_rows > 1]
    ...
    for col in summary.columns:
        if not col.is_numeric and col.unique > 1:
            if col.unique > 1000 or (n_rows > 0 and col.unique / n_rows > 0.5):
                high_card_cols.append(col.name)
    flags["has_high_cardinality_categoricals"] = len(high_card_cols) > 0

    # duplicates
    id_like_cols = [name for name in df.columns if 'id' in name.lower()]
    has_dup_id = False
    for col_name in id_like_cols:
        s = df[col_name]
        if s.notna().sum() > 0:
            # If non-null values are not unique → duplicates
            if s.nunique(dropna=True) != s.notna().sum():
                has_dup_id = True
                break
    flags["has_suspicious_id_duplicates"] = has_dup_id
