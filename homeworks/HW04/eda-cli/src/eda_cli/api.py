...
class QualityFlagsResponse(BaseModel):
    """Полный набор флагов и метрик качества данных, вычисленных по CSV."""

    quality_score: float = Field(..., ge=0.0, le=1.0, description="Интегральная оценка качества (0..1)")
    flags: dict[str, bool | float | int] = Field(
        ...,
        description="Все вычисленные флаги и метрики (булевы, числовые, целочисленные)",
    )
    dataset_shape: dict[str, int] = Field(
        ...,
        description="Размеры датасета: {'n_rows': ..., 'n_cols': ...}",
    )
    latency_ms: float = Field(..., ge=0.0, description="Время обработки в миллисекундах")
...
@app.post(
    ...
    description=(
        "Принимает CSV-файл, выполняет полный EDA-анализ "
        "и возвращает все вычисленные флаги качества данных в формате JSON."
    ),
)
async def quality_flags_from_csv(file: UploadFile = File(...)) -> QualityFlagsResponse:
    """
    Эндпоинт для получения полного набора флагов качества данных по загруженному CSV.
    Использует ядро EDA из модуля core: summarize_dataset, missing_table, compute_quality_flags.
    """
    start = perf_counter()

    # Basic content-type check (lenient)
    if file.content_type not in ("text/csv", "application/vnd.ms-excel", "application/octet-stream"):
    ...
    summary = summarize_dataset(df)
    missing_df = missing_table(df)
    flags_all = compute_quality_flags(summary, missing_df, df)  # ← don't forget `df`!
    ...
    print(
        ...
        f"n_rows={n_rows} n_cols={n_cols} score={flags_all.get('quality_score', 0):.3f} "
        f"latency_ms={latency_ms:.1f} ms"
    )

    return QualityFlagsResponse(
        quality_score=float(flags_all.get("quality_score", 0.0)),
        flags=flags_all,  # includes bools, floats (e.g., max_missing_share), ints
